...
import {
  loadMercadoLivreListingDescription,
...
const errorMessage = (error: unknown): string =>
  error instanceof Error ? error.message : 'Falha ao carregar descrição.';

export async function POST(request: Request) {
  let body: DescriptionRequestBody;
  try {
    body = await request.json() as DescriptionRequestBody;
  } catch {
  ...
  if (!hasMarketplaceCredentials(body.context)) {
    return NextResponse.json(
      {
        error:
          'Contexto Mercado Livre sem user_id ou access_token para consultar a descrição.',
      },
      { status: 400 }
    );
  }
  ...
  try {
    return NextResponse.json(
      await loadMercadoLivreListingDescription(body.context, body.id)
    );
  } catch (error) {
