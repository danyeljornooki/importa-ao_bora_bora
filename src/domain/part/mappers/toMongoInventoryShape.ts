...
const integrationKey = (
  link: PartMarketplaceLink,
  index: number
): string =>
  link.integrationId
  ?? `${link.marketplace}:${link.marketplaceId ?? index}`;
...
const buildIntegrations = (
  ...
  return Object.fromEntries(
    links.map((link, index) => {
      const key = integrationKey(link, index);
...
export const toMongoInventoryShape = (
  part: CanonicalPart
  ...
  marketplace_name_normalized: normalizeName(
    part.identity.marketplaceName ?? part.identity.title
  ),
...
export default toMongoInventoryShape;
