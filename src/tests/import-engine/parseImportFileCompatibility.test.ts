import { describe, expect, it } from 'vitest';
import { runImport } from '../../engine/runImport';
import { parseImportFile } from '../../modules/importer/parseImportFile';
import type {
  ExistingInventoryItem,
  InventoryPersistenceAdapter,
} from '../../types/inventory.types';

const SIMPLE_XLSX_BASE64 =
  '';

const csv = (content: string): Buffer => Buffer.from(content, 'utf-8');

const xlsx = (): Buffer => Buffer.from(SIMPLE_XLSX_BASE64, 'base64');

const toArrayBuffer = (buffer: Buffer): ArrayBuffer => {
  const arrayBuffer = new ArrayBuffer(buffer.byteLength);
  new Uint8Array(arrayBuffer).set(buffer);
  return arrayBuffer;
};

const adapterWithInventory = (
  inventory: ExistingInventoryItem[]
): InventoryPersistenceAdapter => ({
  loadStoreInventory: async () => inventory,
  createItem: async () => ({ success: true, id: 'created' }),
  updateItem: async () => ({ success: true, id: 'updated' }),
});

describe('parseImportFile compatibility', () => {
  it('parseia CSV simples com colunas esperadas', async () => {
    const result = await parseImportFile(
      csv('code,title,price,stock_quantity,location,mlb_ids,image_urls\nP-1,Filtro,10,2,A1,MLB123,http://img'),
      { fileName: 'pecas.csv' }
    );

    expect(result.columns).toEqual([
      'code',
      'title',
      'price',
      'stock_quantity',
      'location',
      'mlb_ids',
      'image_urls',
    ]);
    expect(result.rows[0]).toEqual({
      code: 'P-1',
      title: 'Filtro',
      price: '10',
      stock_quantity: '2',
      location: 'A1',
      mlb_ids: 'MLB123',
      image_urls: 'http://img',
    });
  });

  it('parseia CSV com separador ponto e virgula', async () => {
    const result = await parseImportFile(
      csv('codigo;titulo;preco;estoque\nP-1;Filtro;57,58;4'),
      { fileName: 'pecas.csv' }
    );

    expect(result.columns).toEqual(['codigo', 'titulo', 'preco', 'estoque']);
    expect(result.rows[0]).toEqual({
      codigo: 'P-1',
      titulo: 'Filtro',
      preco: '57,58',
      estoque: '4',
    });
  });

  it('parseia XLSX simples sem depender de xlsx', async () => {
    const result = await parseImportFile(xlsx(), { fileName: 'pecas.xlsx' });

    expect(result.sheetName).toBe('Sheet1');
    expect(result.rows[0]).toMatchObject({
      code: 'P-1',
      title: 'Filtro',
      price: '1.000',
      stock_quantity: '57,58',
      mlb_ids: 'MLB123',
    });
  });

  it('mantem valores como string inicialmente', async () => {
    const result = await parseImportFile(
      csv('code,price,stock_quantity,mlb_ids\n001,"1.000","57,58",MLB123'),
      { fileName: 'pecas.csv' }
    );

    expect(result.rows[0]).toEqual({
      code: '001',
      price: '1.000',
      stock_quantity: '57,58',
      mlb_ids: 'MLB123',
    });
  });

  it('preserva cabecalhos originais', async () => {
    const result = await parseImportFile(
      csv('Codigo Original,Titulo da Peca\nP-1,Filtro'),
      { fileName: 'pecas.csv' }
    );

    expect(result.columns).toEqual(['Codigo Original', 'Titulo da Peca']);
  });

  it('linhas vazias nao quebram a importacao do arquivo', async () => {
    const result = await parseImportFile(
      csv('code,title,price\nP-1,Filtro,10\n,,\nP-2,Farol,20'),
      { fileName: 'pecas.csv' }
    );

    expect(result.totalRows).toBe(2);
    expect(result.rows.map((row) => row.code)).toEqual(['P-1', 'P-2']);
  });

  it('arquivo invalido retorna erro claro', async () => {
    await expect(
      parseImportFile(csv('code,title\nP-1,Filtro'), { fileName: 'pecas.txt' })
    ).rejects.toThrow('Falha ao ler arquivo de importacao');
  });

  it('mantem o plano da engine com creates, updates, skipped, conflicts e invalid', async () => {
    const file = csv([
      'code,title,price,stock_quantity,location',
      'NEW-1,Nova Peca,10,1,A1',
      'UPD-1,Peca Update,20,2,A2',
      'SKIP-1,Peca Igual,30,3,A3',
      'CAND-1,Peca Duplicada,40,4,A4',
      'BAD-1,Peca Invalida,,5,A5',
    ].join('\n'));

    const result = await runImport(toArrayBuffer(file), {
      storeId: 'store-1',
      fileName: 'pecas.csv',
      adapter: adapterWithInventory([
        {
          id: 'existing-update',
          store_id: 'store-1',
          code: 'UPD-1',
          marketplace_name: 'Peca Update',
          price: 19,
          stock_quantity: 2,
          storage_location_name: 'A2',
        },
        {
          id: 'existing-skip',
          store_id: 'store-1',
          code: 'SKIP-1',
          marketplace_name: 'Peca Igual',
          title: 'Peca Igual',
          price: 30,
          stock_quantity: 3,
          storage_location_name: 'A3',
        },
        {
          id: 'existing-conflict',
          store_id: 'store-1',
          code: 'OTHER-1',
          marketplace_name: 'Peca Duplicada',
        },
      ]),
    });

    expect(result.importPlan.summary.creates).toBe(1);
    expect(result.importPlan.summary.updates).toBe(1);
    expect(result.importPlan.summary.skipped).toBe(1);
    expect(result.importPlan.summary.conflicts).toBe(1);
    expect(result.importPlan.summary.invalid).toBe(1);
  });
});
